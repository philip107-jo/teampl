...
export default function Chat({ projectId, projectMembers = [], projectData }: ChatProps) {
  ...
  const [aiTeamSize, setAiTeamSize] = useState<number>(projectMembers?.length || 1);
  ...
  const handleSend = async () => {
    ...
    if (inputText.startsWith("/ai ")) {
      ...
      setAiLoading(true);
      try {
        const suggestions = await aiApi.splitTasks(projectId, aiTeamSize, "자유 요청", desc);
  ...
  const handleAiAnalysis = async () => {
    if (!aiTopic.trim() || !projectId) {
      alert("프로젝트 유형/주제를 입력해주세요.");
      return;
    }
    setAiLoading(true);
    setIsAnalyzing(true);
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <label className="text-xs font-black tracking-widest text-[#7D879C] uppercase ml-1">참여 인원 수 (명)</label>
                      <input 
                        type="number" 
                        min="1" max="20"
                        value={aiTeamSize}
                        onChange={(e) => setAiTeamSize(parseInt(e.target.value) || 1)}
                        className="w-full p-4 bg-white dark:bg-[#0d1526] border border-gray-200 dark:border-white/10 rounded-2xl focus:border-[#7C6CFF] outline-none font-medium dark:text-white"
                      />
                    </div>
                    <div className="space-y-2">
                      <label className="text-xs font-black tracking-widest text-[#7D879C] uppercase ml-1">프로젝트 유형 / 주제</label>
                      <input 
                        type="text" 
                        placeholder="예: 웹 애플리케이션 개발, 발표 등"
                        value={aiTopic}
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <label className="text-xs font-black tracking-widest text-[#7D879C] uppercase ml-1">과제 세부 설명 (선택)</label>
                      <span className="text-[10px] font-black px-2 py-0.5 bg-[#7C6CFF]/10 text-[#7C6CFF] rounded-lg">AI가 팀의 초기 세팅을 도와줍니다</span>
                    </div>
                    <textarea 
                      className="w-full h-32 p-4 bg-white dark:bg-[#0d1526] border border-gray-200 dark:border-white/10 rounded-2xl focus:border-[#7C6CFF] focus:shadow-[0_0_20px_rgba(124,108,255,0.15)] outline-none transition-all dark:text-white font-medium resize-none placeholder:text-[#7D879C]/40"
