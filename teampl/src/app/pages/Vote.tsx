...
export default function VotePage({ projectId, isReadOnly }: VoteProps) {
  ...
  const [voteToDelete, setVoteToDelete] = useState<Vote | null>(null);
  ...
  const handleDeleteVoteClick = (vote: Vote) => {
        ...
        <div className="space-y-4">
          ...
          {activeVotes.map(vote => (
            <VoteCard
              ...
              onDelete={handleDeleteVoteClick}
        ...
        <div className="fixed inset-0 z-[250] flex items-center justify-center p-4 bg-black/80 backdrop-blur-xl animate-in fade-in duration-300">
          <div className="card w-full max-w-[400px] shadow-[0_30px_60px_rgba(0,0,0,0.6)] !p-8 border border-red-500/20 dark:bg-[#132038]">
            ...
            <p className="text-[13px] font-bold text-center text-[#7D879C]/80 dark:text-white/40 mb-6 break-keep leading-relaxed">
              <span className="text-[#1A2340] dark:text-white">'{voteToDelete.title}'</span> 투표를 삭제하시겠습니까? 삭제된 투표 정보와 내역은 복구할 수 없습니다.
            </p>
