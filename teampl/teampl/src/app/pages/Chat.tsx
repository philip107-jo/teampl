...
import { currentUser, initialMembers as globalMembers } from "../mockData";
import {
  Send,
  Plus,
  MoreVertical,
...
export default function Chat() {
  ...
  useEffect(() => {
    // 채팅방 데이터가 아예 없을 때(새 프로젝트) 초기 안내 메시지 세팅
    if (navStep === "CHAT" && chatMode === "TEAM" && selectedProjectId) {
      setMessagesStore(prev => {
        const currentMsgs = prev[currentChatKey];
        if (!currentMsgs || currentMsgs.length === 0) {
          const aiWelcomeMessage: Message = {
            id: "ai-init",
            sender: "TeamSync AI",
            content: "안녕하세요! 팀 프로젝트 협업을 도와드릴 AI 비서입니다. 🤖\n\n원활한 프로젝트 진행을 위해 먼저 수행하실 과제 내용이나 기획안을 올려주세요. 제가 최적의 일정과 역할을 제안해 드리겠습니다! ✨",
            time: new Date().toLocaleTimeString('ko-KR', { hour: 'numeric', minute: '2-digit', hour12: true }),
            isMe: false
          };
          return { ...prev, [currentChatKey]: [aiWelcomeMessage] };
        }
        return prev;
      });
    }
  ...
  }, [navStep, currentChatKey, chatMode, selectedProjectId]); // messagesStore를 의존성에서 제거하여 무한 루프 위험 제거
  ...
  const handleSend = () => {
    ...
    setMessagesStore(prev => ({
      ...prev,
      [currentChatKey]: updatedMessages
    }));

    // AI 제안 트리거: 팀 채널이면서 아직 AI의 제안(분석 답변)이 없는 경우
    const hasAIAnalysis = updatedMessages.some(m => !m.isMe && m.id?.includes("-ai"));
    if (chatMode === "TEAM" && !isAILoading && !hasAIAnalysis) {
      setIsAILoading(true);
      
      chatApi.suggestAssignment(inputText)
        .then(suggestion => {
          ...
          setMessagesStore(prev => ({
            ...prev,
            [currentChatKey]: [...(prev[currentChatKey] || []), aiReply]
          }));
        })
        .catch(err => {
          const errorMsg: Message = {
            ...
            sender: "System",
            content: "죄송합니다. AI 분석 중 오류가 발생했습니다. 잠시 후 다시 시도해 주세요.",
            time: new Date().toLocaleTimeString('ko-KR', { hour: 'numeric', minute: '2-digit', hour12: true }),
            isMe: false
          };
          setMessagesStore(prev => ({
            ...prev,
            [currentChatKey]: [...(prev[currentChatKey] || []), errorMsg]
          }));
        })
        .finally(() => {
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {projectsList.map((project) => (
                ...
                >
                  <div className={`schedule-item ${project.theme || project.color || 'blue'} !p-0 !border-none bg-transparent flex-shrink-0`}>
                    <div className="schedule-icon" style={{ width: 56, height: 56, borderRadius: 16 }}>
                      {typeof project.icon === 'string' || !project.icon ? <Database className="w-7 h-7 text-[#1A2340] dark:text-white" /> : <project.icon className="w-7 h-7 text-[#1A2340] dark:text-white" />}
                    </div>
                  </div>
  ...
  // 3단계: 채팅창
  return (
    <div className="flex flex-col h-[calc(100vh-180px)] md:h-[calc(100vh-220px)] bg-[#f8faff] dark:bg-[#0B1020] overflow-hidden relative transition-all duration-300 rounded-[32px] border border-gray-200 dark:border-white/5 shadow-xl">
      <div className="p-4 md:p-6 border-b border-gray-200 dark:border-white/5 flex items-center justify-between bg-white dark:bg-[#12182B]/90 backdrop-blur-md sticky top-0 z-10 transition-all">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-4">
            {chatMode === "TEAM" ? (
              <div className={`schedule-item ${selectedProject?.theme || selectedProject?.color || 'blue'} !p-0 !border-none bg-transparent flex-shrink-0`}>
                <div className="schedule-icon" style={{ width: 48, height: 48, borderRadius: 14 }}>
                  {selectedProject && (typeof selectedProject.icon === 'string' || !selectedProject.icon ? <Database className="w-6 h-6 text-[#1A2340] dark:text-white" /> : <selectedProject.icon className="w-6 h-6 text-[#1A2340] dark:text-white" />)}
                </div>
              </div>
            ) : (
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 space-y-8 transition-all scrollbar-hide">
        {currentMessages.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-center p-10 space-y-6">
            <div className={`schedule-item ${chatMode === "TEAM" ? (selectedProject?.theme || selectedProject?.color || 'blue') : "purple"} !p-0 !border-none bg-transparent flex-shrink-0 animate-in zoom-in-50 duration-500`}>
              <div className="schedule-icon" style={{ width: 112, height: 112, borderRadius: 40 }}>
                {chatMode === "TEAM" ? (
                  selectedProject && (typeof selectedProject.icon === 'string' || !selectedProject.icon ? <Database className="w-12 h-12 text-[#1A2340] dark:text-white" /> : <selectedProject.icon className="w-12 h-12 text-[#1A2340] dark:text-white" />)
                ) : (
            ...
            <div key={msg.id} className={`flex ${msg.isMe ? "justify-end" : "justify-start"} items-start gap-4 animate-in slide-in-from-bottom-2`}>
               {!msg.isMe && (
                <div className={`w-10 h-10 rounded-[14px] ${
                  msg.sender === "TeamSync AI" 
                    ? "bg-gradient-to-br from-[#7C6CFF] to-[#4D8DFF] shadow-[0_0_20px_rgba(124,108,255,0.4)]" 
                    : (members.find(m => m.name === msg.sender)?.avatarColor || "bg-[#7C6CFF]")
                } text-white flex items-center justify-center text-[15px] font-black shadow-[0_0_15px_rgba(0,0,0,0.2)] flex-shrink-0 mt-1 uppercase`}>
                  {msg.sender === "TeamSync AI" ? <Bot className="w-6 h-6" /> : msg.sender[0]}
                </div>
              )}
              <div className={`max-w-[70%] space-y-2 ${msg.isMe ? "flex flex-col items-end" : ""}`}>
                ...
                <div className={`flex items-end gap-3 ${msg.isMe ? "flex-row-reverse" : "flex-row"}`}>
                  <div className={`px-5 py-3.5 rounded-[24px] text-[15px] font-medium transition-all shadow-sm leading-relaxed break-words whitespace-pre-wrap ${msg.isMe ? `${currentUser.avatarColor || "bg-[#7C6CFF]"} text-white rounded-br-none shadow-[0_0_20px_rgba(124,108,255,0.4)] border border-[#7C6CFF]/50` : "bg-white/40 dark:bg-[#1A2340] text-[#1A2340] dark:text-white rounded-bl-none border border-gray-300 dark:border-white/10"}`}>
                    {msg.content}
                  </div>
        ...
        {isAILoading && (
          <div className="flex justify-start items-start gap-4 animate-in fade-in slide-in-from-bottom-2">
            <div className={`w-10 h-10 rounded-[14px] bg-gradient-to-br from-[#7C6CFF] to-[#4D8DFF] shadow-[0_0_20px_rgba(124,108,255,0.4)] text-white flex items-center justify-center shadow-[0_0_15px_rgba(0,0,0,0.2)] flex-shrink-0 mt-1`}>
              <Bot className="w-6 h-6" />
            </div>
            <div className="max-w-[70%] space-y-2">
              <p className="text-[12px] font-black text-[#7D879C] dark:text-white/80 ml-1 uppercase tracking-widest">TeamSync AI</p>
              <div className="flex items-center gap-3">
                <div className="px-5 py-4 bg-white/40 dark:bg-[#1A2340] text-white rounded-[24px] rounded-bl-none border border-gray-300 dark:border-white/10 flex items-center gap-3">
                  <Loader2 className="w-5 h-5 animate-spin text-[#7C6CFF]" />
                  <span className="text-[14px] font-medium text-[#1A2340] dark:text-white/80">과제 내용을 분석 중입니다...</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="p-5 bg-white dark:bg-[#12182B] border-t border-gray-200 dark:border-white/5 transition-all">
        <div className="flex items-center gap-3 bg-white/40 dark:bg-[#1A2340] border border-gray-300 dark:border-white/10 rounded-3xl pl-3 pr-2 py-2 group focus-within:bg-white/40 dark:bg-[#1A2340] focus-within:border-[#7C6CFF]/50 focus-within:shadow-[0_0_15px_rgba(124,108,255,0.1)] transition-all">
          ...
          <input
            type="text"
            placeholder="메시지 입력..."
            className="flex-1 bg-transparent border-none focus:ring-0 text-[15px] font-medium py-2 placeholder-[#7D879C]/50 text-[#1A2340] dark:text-white outline-none"
            value={inputText}
